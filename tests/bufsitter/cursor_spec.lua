...
describe("cursor", function()
  ...
  describe("root()", function()
    it("should have exec method", function()
      assert.are.same(true, type(cursor.root().exec) == "function")
    end)

    it("should return empty when parser fails", function()
      local bufnr = vim.api.nvim_create_buf(false, true)
      local items = cursor.root():exec(bufnr)
    ...
    it("should return root node and full range", function()
      ...
      assert.are.same(expected_root, items[1])
      local sr, sc, er, ec = items[1]:range()
      assert.are.same(0, sr)
      assert.are.same(0, sc)
      assert.are.same(2, er)
      assert.are.same(0, ec)
    end)
  end)

  describe("query()", function()
    it("should have exec method", function()
      assert.are.same(true, type(cursor.query("(section) @node").exec) == "function")
    end)

    it("should return all matching nodes", function()
      local bufnr = h.make_buf({ "# A", "# B", "# C" }, "markdown")
      local items = cursor.query("(section) @node"):exec(bufnr)
    ...
    it("should return empty when query matches nothing", function()
      ...
      local items = cursor.query("(fenced_code_block) @node"):exec(bufnr)
      assert.are.same(0, #items)
    end)
    ...
    it("should error on invalid query", function()
      ...
      assert.has_error(function()
        cursor.query("this is not valid"):exec(bufnr)
      end)
    end)

    it("chains into cursor methods", function()
      local bufnr = h.make_buf({ "# A", "# B" }, "markdown")
      local items = cursor.query("(section) @node"):first():exec(bufnr)
      assert.are.same(1, #items)
      assert.are.same("section", items[1]:type())
    end)
  end)

  describe("children()", function()
    it("should have exec method", function()
      assert.are.same(true, type(cursor.root():children().exec) == "function")
    end)
    ...
    it("filters by type", function()
      local bufnr = h.make_buf({ "# A", "# B" }, "markdown")
      local items = cursor.root():children({ types = { "section" } }):exec(bufnr)
      assert.are.same(2, #items)
      for _, node in ipairs(items) do
        assert.are.same("section", node:type())
      end
    end)
    ...
    it("filters by multiple types (OR)", function()
      local bufnr = h.make_buf({ "# A", "# B" }, "markdown")
      local items =
        cursor.root():children({ types = { "section", "atx_heading" } }):exec(bufnr)
    ...
    it("returns empty when type filter matches nothing", function()
      local bufnr = h.make_buf({ "# Title" }, "markdown")
      local items =
        cursor.root():children({ types = { "fenced_code_block" } }):exec(bufnr)
      assert.are.same(0, #items)
    end)
    ...
    it("returns empty when field does not exist", function()
      local bufnr = vim.api.nvim_create_buf(false, true)
      local items = cursor.root():first():children({ names = { "name" } }):exec(bufnr)
      assert.are.same(0, #items)
    end)
    ...
    it("type and name opts are AND", function()
      ...
      local items = cursor
        ...
        :children({ types = { "section" } })
        :first()
        :children({ names = { "body" }, types = { "atx_heading" } })
        :exec(bufnr)
      assert.are.same(0, #items)
    end)
    ...
    it("chains: children({types}):first():children({types})", function()
      ...
      local items = cursor
        ...
        :exec(bufnr)
      assert.are.same(1, #items)
      assert.are.same("atx_heading", items[1]:type())
    end)
  ...
  describe("parent()", function()
    ...
    it("should return empty when node has no parent", function()
      local bufnr = h.make_buf({ "# Title" }, "markdown")
      local items = cursor.root():first():parent():exec(bufnr)
      assert.are.same(0, #items)
    end)
    ...
    it("or_else() falls back to node when parent is empty", function()
      local bufnr = h.make_buf({ "# Title" }, "markdown")
      local items =
        cursor.root():first():parent({ types = { "nonexistent" } }):or_else():exec(bufnr)
      assert.are.same(1, #items)
      assert.are.same("document", items[1]:type())
    end)
  end)
  ...
  describe("siblings()", function()
    it("should have exec method", function()
      ...
      assert.are.same(
        true,
        type(cursor.root():children({ types = { "section" } }):first():siblings().exec)
          == "function"
      )
    end)
    ...
    it("should return all siblings excluding self", function()
      local bufnr = h.make_buf({ "# A", "# B", "# C" }, "markdown")
      local items =
        cursor.root():children({ types = { "section" } }):first():siblings():exec(bufnr)
      assert.are.same(2, #items)
    end)
    ...
    it("should return empty when node has no parent", function()
      local bufnr = h.make_buf({ "# Title" }, "markdown")
      local items = cursor.root():first():siblings():exec(bufnr)
      assert.are.same(0, #items)
    end)
  end)
  ...
  describe("next_siblings()", function()
    it("should have exec method", function()
      ...
      assert.are.same(
        true,
        type(cursor.root():children():first():next_siblings().exec) == "function"
      )
    end)

    it("should return all next siblings", function()
      local bufnr = h.make_buf({ "# A", "# B", "# C" }, "markdown")
      local items = cursor.root():children():first():next_siblings():exec(bufnr)
      assert.are.same(2, #items)
    end)
    ...
    it("should return empty when no next siblings exist", function()
      local bufnr = h.make_buf({ "# A", "# B", "# C" }, "markdown")
      local items = cursor.root():children():last():next_siblings():exec(bufnr)
      assert.are.same(0, #items)
    end)
  end)
  ...
  describe("prev_siblings()", function()
    it("should have exec method", function()
      ...
      assert.are.same(
        true,
        type(cursor.root():children():last():prev_siblings().exec) == "function"
      )
    end)

    it("should return all prev siblings", function()
      local bufnr = h.make_buf({ "# A", "# B", "# C" }, "markdown")
      local items = cursor.root():children():last():prev_siblings():exec(bufnr)
      assert.are.same(2, #items)
    end)
    ...
    it("should return empty when no prev siblings exist", function()
      local bufnr = h.make_buf({ "# A", "# B", "# C" }, "markdown")
      local items = cursor.root():children():first():prev_siblings():exec(bufnr)
      assert.are.same(0, #items)
    end)
  end)
  ...
  describe("or_else()", function()
    ...
    it("double or_else is no-op", function()
      ...
      local once = base:or_else():exec(bufnr)
      local twice = base:or_else():or_else():exec(bufnr)
  ...
  describe("flatMap traversal", function()
    it("children() collects children from all nodes", function()
      ...
      local items = cursor.root():children():children():exec(bufnr)
    ...
    it("children({types}) filters across all nodes", function()
      ...
      local items =
        cursor.root():children():children({ types = { "atx_heading" } }):exec(bufnr)
      assert.are.same(2, #items)
      for _, node in ipairs(items) do
        assert.are.same("atx_heading", node:type())
      end
    end)
  ...
  describe("nth()", function()
    it("should return nth item (1-based)", function()
      local bufnr = h.make_buf({ "# A", "# B", "# C" }, "markdown")
      local items = cursor.root():children():nth(2):exec(bufnr)
      assert.are.same(1, #items)
      assert.are.same(h.get_root(bufnr):named_child(1), items[1])
    end)

    it("should support negative index (-1 = last)", function()
      local bufnr = h.make_buf({ "# A", "# B", "# C" }, "markdown")
      local items = cursor.root():children():nth(-1):exec(bufnr)
      local root = h.get_root(bufnr)
      assert.are.same(root:named_child(2), items[1])
    end)

    it("should return empty when index out of range", function()
      local bufnr = h.make_buf({ "# A" }, "markdown")
      local items = cursor.root():children():nth(5):exec(bufnr)
      assert.are.same(0, #items)
    end)
  ...
  describe("first() / last()", function()
    it("first() returns first item", function()
      local bufnr = h.make_buf({ "# A", "# B", "# C" }, "markdown")
      local items = cursor.root():children():first():exec(bufnr)
      assert.are.same(1, #items)
      assert.are.same(h.get_root(bufnr):named_child(0), items[1])
    end)
    ...
    it("last() returns last item", function()
      ...
      local root = h.get_root(bufnr)
      assert.are.same(root:named_child(root:named_child_count() - 1), items[1])
    end)
  end)
  ...
  describe("assert()", function()
    describe("Multi:assert()", function()
      it("passes through nodes when non-empty", function()
        local bufnr = h.make_buf({ "# A", "# B" }, "markdown")
        local items = cursor.root():children():assert():exec(bufnr)
        assert.are.same(2, #items)
      end)
      ...
      it("errors with default message when empty", function()
        local bufnr = h.make_buf({ "# Title" }, "markdown")
        assert.has_error(function()
          cursor.root():children({ types = { "nonexistent" } }):assert():exec(bufnr)
        end)
      end)
      ...
      it("is chainable", function()
        local bufnr = h.make_buf({ "# A", "# B" }, "markdown")
        local items = cursor.root():children():assert():first():exec(bufnr)
        assert.are.same(1, #items)
      end)
    end)
    ...
    describe("Single:assert()", function()
      it("passes through node when present", function()
        local bufnr = h.make_buf({ "# Title" }, "markdown")
        local items = cursor.root():children():first():assert():exec(bufnr)
        assert.are.same(1, #items)
      end)
      ...
      it("is chainable", function()
        local bufnr = h.make_buf({ "# Title", "content" }, "markdown")
        local items = cursor.root():children():first():assert():children():exec(bufnr)
