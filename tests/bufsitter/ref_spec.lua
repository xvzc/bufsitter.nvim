...
describe("ref", function()
  ...
  describe("buffer", function()
    it("should return [No Name] when buffer has no name", function()
      assert.are.same("[No Name]", ref.get())
    end)
    ...
    it("should return home-relative path by default", function()
      ...
      assert.are.same(expected, ref.get())
    end)
    ...
    it("should return home-relative path when expand=false", function()
      ...
      assert.are.same(expected, ref.get({ expand = false }))
    end)
    ...
    it("should return absolute path when expand=true", function()
      ...
      assert.are.same(expected, ref.get({ expand = true }))
    end)
  end)
  ...
  describe("visual_selection", function()
    ...
    it("should return single line ref for same-line selection", function()
      ...
      assert.are.same(buf_name .. ":L5", ref.get())
    end)
    ...
    it("should return range ref for multi-line selection", function()
      ...
      assert.are.same(buf_name .. ":L3~L7", ref.get())
    end)
    ...
    it("should use home-relative path by default", function()
      ...
      assert.are.same(buf_name .. ":L1~L2", ref.get())
    end)
    ...
    it("should use absolute path when expand=true", function()
      ...
      assert.are.same(buf_name .. ":L1~L3", ref.get({ expand = true }))
    end)
  end)
  ...
  describe("get", function()
    ...
    it("should apply hook to result before returning", function()
      local name = set_buf_name(vim.fn.tempname())
      local actual = ref.get({
        hook = function(r)
          return "HOOKED:" .. r
        end,
      })
      local expected = "HOOKED:" .. vim.fn.fnamemodify(name, ":~")
      assert.are.same(expected, actual)
    end)

    it("should not call hook when hook is nil", function()
      local name = set_buf_name(vim.fn.tempname())
      local expected = vim.fn.fnamemodify(name, ":~")
      assert.are.same(expected, ref.get())
    end)

    it("should allow hook to transform path in visual mode", function()
      vim.api.nvim_get_mode = function()
        return { mode = "v" }
      end
      local name = set_buf_name(vim.fn.tempname())
      ...
      assert.are.same((buf_name .. ":L2~L4"):upper(), actual)
    end)
  end)
end)
